...
> PowerShell_경로처리_가이드.md를 항상 확인하도록 설정된 상태
...
### 1. **VS Code Workspace 설정** (`root.code-workspace`)
   - ✅ 경로 처리 주의사항 주석 추가
   - ✅ 참고 문서 명시: `PowerShell_경로처리_가이드.md`
   - ✅ 핵심 원칙 표시
...
```powershell
# 경로 처리 가이드 열기
code PowerShell_경로처리_가이드.md
```

**VS Code 파일 탐색기에서:**
- `PowerShell_경로처리_가이드.md` 파일을 클릭하면 즉시 열림
...
- [ ] 경로에 대괄호 `[ ]`가 있나? → **PowerShell_경로처리_가이드.md 참고!**
...
$file = Join-Path $path "Contents\인트로.mars"
Expand-Archive -Path $file -DestinationPath $extract

# ❌ 잘못된 방법
cd "C:\[999999-99][폴리텍전기콘텐츠프로젝트]\..."  # 실패!
Expand-Archive -Path "C:\...\Contents\인트로.mars"  # 위험!
```
...
| 가이드 | 파일 | 용도 |
|--------|------|------|
| **경로 처리** | `PowerShell_경로처리_가이드.md` | ⭐ **작업 전 필수** |
| **콘텐츠 구조** | `인트로_콘텐츠_구조.md` | 콘텐츠 분석 시 |
...
**File Explorer에서:**
```
프로젝트 폴더 → PowerShell_경로처리_가이드.md → 더블클릭
```

**명령줄에서:**
```bash
# 모든 가이드 한 번에 VS Code에서 열기
code PowerShell_경로처리_가이드.md AGENTS.md 인트로_콘텐츠_구조.md
```
